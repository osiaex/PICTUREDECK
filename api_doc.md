...
| **内容管理** | `/api/v1/user/generation_list/{taskId}` | `DELETE`  | [删除生成记录](#删除生成记录接口)   
| **内容管理** | `/api/v1/user/favorite_list/batch`       | `POST`  | [批量新增收藏夹内容](#批量新增收藏夹内容接口)                       |
| **NFT管理** | `/api/v1/nft/mint`       | `POST`  | [铸造NFT](#铸造NFT接口)                       |
| **NFT管理** | `/api/v1/nft/transfer`       | `POST`  | [转移NFT](#转移NFT接口)                       |
| **NFT管理** | `/api/v1/nft/mint/{token_id}`       | `GET`  | [查询NFT铸造结果](#查询NFT铸造结果接口)                       |
| **NFT管理** | `/api/v1/nft/transfer/{token_id}`       | `GET`  | [查询NFT转移结果](#查询NFT转移结果接口)                       |
...
- **URI**: `/api/v1/user/generation_list/{taskId}`
- **方法**: `POST`
...
| :----- | :--- | :----- | :--------------------------------------------------------------------------------------- |
| `code` | int  | 状态码 | 200（成功）；400（失败，参数不合规）；401（失败，鉴权失败）；500（失败，服务器内部错误） |
| `data.task_id` | obj  | 被删除的生成记录的task_id |  |
...
### 17. 铸造NFT接口<span id="铸造NFT接口"></span>

- **URI**: `/api/v1/nft/mint`
- **方法**: `POST`
- **功能**: 后端为一张图片代为铸造 NFT，铸造的NFT归后端的统一钱包所持有
...
```json
{
  ...
  "task_id": "8fb2f120-b1b4-4331-acbf-e3a2f0f9d036"
}
...
| 参数名      | 类型   | 说明   | 约束 |
| :---------- | :----- | :----- | :--- |
| `name` | string | 图片作为nft的别名    |
| `description` | string | 对图片的描述 |  |
| `task_id` | string | 要铸造NFT的图片对应的task_id |
...
| 参数名 | 类型 | 说明   | 约束                                                                                     |
| :----- | :--- | :----- | :--------------------------------------------------------------------------------------- |
| `code` | int  | 状态码 | 200（成功）；400（失败，参数不合规）；401（失败，鉴权失败）；402（失败，用户并未持有该生成请求）；403（失败，无有效生成结果）；500（失败，服务器内部错误） |
| data.mint_task_id | string  | 铸造任务的id |  |

### 18. 转移NFT接口<span id="转移NFT接口"></span>

- **URI**: `/api/v1/nft/transfer`
- **方法**: `POST`
- **功能**: 把铸好的 NFT 转移给用户
...
| 参数名      | 类型   | 说明   | 约束 |
| :---------- | :----- | :----- | :--- |
| `token_id` | string | nft的token_id| 在服务器中唯一存在     |
| `to_address` | string | 目的地址 |   |
...
| 参数名 | 类型 | 说明   | 约束                                                                                     |
| :----- | :--- | :----- | :--------------------------------------------------------------------------------------- |
| `code` | int  | 状态码 | 200（成功）；400（失败，参数不合规）；401（失败，鉴权失败）；402（失败，用户并非NFT所有者）；403（失败，NFT不存在）；500（失败，服务器内部错误） |
| data.token_id | string  | 要转移的NFT的token |  |
| data.to_address | string  | 转移的目标地址 |  |



### 19. 查询NFT铸造结果接口<span id="查询NFT铸造结果接口"></span>

- **URI**: `/api/v1/nft/mint/{mint_task_id}`
- **方法**: `GET`
- **功能**: 查询NFT铸造任务的结果
...
```json
{
  ...
  "data": {
    "status": "minted",
    "token_id": "1",
    "owner_address": "0xUserWalletAddress"
  }
...
| :----- | :--- | :----- | :--------------------------------------------------------------------------------------- |
| `code` | int  | 状态码 | 200（成功）；400（失败，参数不合规）；401（失败，鉴权失败）；402（失败，用户并未持有该生成请求）；403（失败，无有效生成结果）；500（失败，服务器内部错误） |
| data.status | string  | 铸造NFT任务的状态 | 一定是"pending_mint"或者"minted" |
| data.token_id | string  | 铸造好的NFT的token_id，仅当任务完成时存在 |  |
| data.owner_address | string  | nft暂时所属的地址，仅当任务完成时存在 |  |


### 20. 查询NFT转移结果接口<span id="查询NFT转移结果接口"></span>

- **URI**: `/api/v1/nft/transfer/{transfer_task_id}`
- **方法**: `GET`
- **功能**: 查询NFT转移任务的结果
...
| :----- | :--- | :----- | :--------------------------------------------------------------------------------------- |
| `code` | int  | 状态码 | 200（成功）；400（失败，参数不合规）；401（失败，鉴权失败）；402（失败，用户并未持有该生成请求）；403（失败，无有效生成结果）；500（失败，服务器内部错误） |
| data.status | string  | 铸造NFT任务的状态 | 一定是"transfer_pending"或者"transferred" |
| data.token_id | string  | 要转移的NFT的token |  |
| data.to_address | string  | 转移的目标地址 |  |
